  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(result.message || "Failed to add contest");
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
        ...
        <textarea {...register("description", { required: true })} placeholder="Description" className="textarea" />
        <input type="number" {...register("price", { required: true })} placeholder="Price" className="input" />
        <input type="number" {...register("prize", { required: true })} placeholder="Prize Money" className="input" />
        <textarea {...register("task", { required: true })} placeholder="Task Instruction" className="textarea" />
